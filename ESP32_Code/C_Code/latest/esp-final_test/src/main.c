...
sg90_servo_t servoX = { .channel = SERVO_X_CHANNEL, .timer = LEDC_TIMER_1, .speed_mode = LEDC_LOW_SPEED_MODE, .gpio_num = SERVO_X_PIN };
sg90_servo_t servoY = { .channel = SERVO_Y_CHANNEL, .timer = LEDC_TIMER_1, .speed_mode = LEDC_LOW_SPEED_MODE, .gpio_num = SERVO_Y_PIN };
sg90_servo_t servoZ = { .channel = SERVO_Z_CHANNEL, .timer = LEDC_TIMER_1, .speed_mode = LEDC_LOW_SPEED_MODE, .gpio_num = SERVO_Z_PIN };
sg90_servo_t servoG = { .channel = SERVO_G_CHANNEL, .timer = LEDC_TIMER_1, .speed_mode = LEDC_LOW_SPEED_MODE, .gpio_num = SERVO_GRAB };
...
volatile bool siren_active = true;  // Flag to track button press state
...
void setup_switch() {
    ...
    servo_pos(&servoX, 10);  // Neutral (middle)
    servo_pos(&servoY, 10);  // Neutral (middle)
    servo_pos(&servoZ, 10);  // Neutral toggle position
    servo_pos(&servoG, 20);  // Neutral toggle position
}
    ...
    void control_servoZ_toggle(uint8_t switch_value, sg90_servo_t *servo) {
        if (switch_value == 0xFF) {
            // Button released
            return;
        }
        else if (switch_value == 0x00) {
            // Button pressed -> toggle servo
            servoZ_toggled = !servoZ_toggled;
 
            int pos = servoZ_toggled ? 15 : 5;  // Example: open (position 5) / close (position 15)
            if (servo->gpio_num == 40){
                ESP_LOGI(TAG, "New Position: %d, Servo GPIO: %d", pos, servo->gpio_num);
            }
            servo_pos(servo, pos);
        }
    }
    ...
    void control_servoG_toggle(uint8_t switch_value, sg90_servo_t *servo) {
        ...
        if (switch_value == 0xFF) {
            ...
            // Button pressed -> toggle servo
            servoG_toggled = !servoG_toggled;
 
            int pos = servoG_toggled ? 20 : 1;  // Example: open (position 5) / close (position 15)
            if (servo->gpio_num == 39){
                ESP_LOGI(TAG, "New Position: %d, Servo GPIO: %d", pos, servo->gpio_num);
            }
            servo_pos(servo, pos);
        }
    ...
    void control_servo(uint8_t servo_value, sg90_servo_t *servo, int *current_pos) {
        ...
        // if (servo->gpio_num == 41){
        //     ESP_LOGI(TAG, "Old Position: %d, Servo GPIO: %d", *current_pos, servo->gpio_num);
        // }
        ...
        // if (servo->gpio_num == 41){
        //     ESP_LOGI(TAG, "New Position: %d, Servo GPIO: %d", *current_pos, servo->gpio_num);
        // }
        // Apply the position
...
void handle_nrf_data() {
    ...
    //     NRF24_Receive(RxData);
        ...
        // uint8_t servoX_data = RxData[2];
        // uint8_t servoY_data = RxData[3];
        // uint8_t servoSwitch = RxData[5];
        // uint8_t grabButton = RxData[4];
...
{
    bool current_state = gpio_get_level(SWITCH_PIN);  // Read switch state (HIGH or LOW)

    switch_pressed = (current_state == 0);  // LOW = switch ON (because pull-up is enabled)

    // Optional log if you want
    // ESP_LOGI(TAG, "Switch is now: %s", switch_pressed ? "ON" : "OFF");
}
        // siren_active = true;
...
uint8_t grabButton;
        ...
        while(1)
        {
            ...
            servoSwitch = 0xFF;
            grabButton = 0xFF;
            ESP_LOGI(TAG, "Servo X: %d, Y: %d", servoX_data, servoY_data);
            for (int i = 0; i< 5; i++){
                control_servo(servoX_data, &servoX, &servoX_pos);
                control_servo(servoY_data, &servoY, &servoY_pos);
        
                // 🛠 Toggle servo Z and Grab
                control_servoZ_toggle(servoSwitch, &servoZ);
                control_servoG_toggle(grabButton, &servoG);
