...
#define SERVO_NEUTRAL_DUTY 491  // Duty cycle for 90 degrees (1.5 ms pulse)
    ...
    void setup_servo() {
        ...
        ledc_timer_config_t booster_timer = {
            .speed_mode = LEDC_LOW_SPEED_MODE,
            .timer_num = LEDC_TIMER_2,  // Use a different timer for the servos
            .duty_resolution = BOOSTER_RES,
            .freq_hz = BOOSTER_FREQ,
            .clk_cfg = LEDC_AUTO_CLK
        };
        ledc_timer_config(&booster_timer);
        ...
        ledc_channel_config_t servoX_channel = {
            ...
            .duty = SERVO_NEUTRAL_DUTY,  // Start at neutral position
            .hpoint = 0
        };
        ledc_channel_config(&servoX_channel);

        ledc_channel_config_t booster = {
            .gpio_num = BOOSTER_PIN,
            .speed_mode = LEDC_LOW_SPEED_MODE,
...
void control_servo(uint8_t servo_value, ledc_channel_t servo_channel) {
    int duty_cycle = SERVO_NEUTRAL_DUTY;  // Default to neutral position

    if (servo_value < 0x80) {
        // Move counterclockwise (left)
        duty_cycle = SERVO_NEUTRAL_DUTY - ((0x80 - servo_value) * 2);  // Adjust multiplier for speed
    } else if (servo_value > 0x80) {
        // Move clockwise (right)
        duty_cycle = SERVO_NEUTRAL_DUTY + ((servo_value - 0x80) * 2);  // Adjust multiplier for speed
    }

    // Clamp the duty cycle to valid range
    duty_cycle = (duty_cycle < SERVO_MIN_DUTY) ? SERVO_MIN_DUTY : (duty_cycle > SERVO_MAX_DUTY) ? SERVO_MAX_DUTY : duty_cycle;

    // Set the duty cycle for the specified servo channel
    ledc_set_duty(LEDC_LOW_SPEED_MODE, servo_channel, duty_cycle);
