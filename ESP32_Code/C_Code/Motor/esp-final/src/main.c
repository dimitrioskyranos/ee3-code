...
#define SERVO_MIN_DUTY_G 200  // Duty cycle for 0 degrees (1 ms pulse)
...
static void uart_init(void) {
    ...
    xTaskCreatePinnedToCore(uart_event_task, "uart_event_task", 8192, NULL, 5, NULL, 1);
...
void handle_nrf_data() {
    ...
    //     // Optional logging
       ESP_LOGI(TAG, "Joystick X: %d, Y: %d, Switch: %d", joystickX, joystickY, grabButton);
       ESP_LOGI(TAG, "Servo X: %d, Y: %d, Switch: %d", servoX, servoY, servoSwitch);
    }
}
...
void check_switch_toggle()
{
    ...
    nrf_timer = xTimerCreate("nrf_timer", pdMS_TO_TICKS(100), pdTRUE, NULL, nrf_timer_callback);
    if (nrf_timer != NULL) {
