...
static void uart_init(void) {
    ...
    uart_driver_install(UART_NUM, BUF_SIZE * 8, BUF_SIZE * 8, QUEUE_SIZE, &uart_queue, 0);
...
void setup_switch() {
    ...
    servo_pos(&servoG, 5);  // Neutral toggle position
}
    ...
    void control_servoG_toggle(uint8_t switch_value, sg90_servo_t *servo) {
        if (switch_value == 0xFF) {
            ...
            int pos = servoG_toggled ? 15 : 5;  // Example: open (position 5) / close (position 15)
            servo_pos(servo, pos);
        }
    }
...
void handle_nrf_data() {
    ...
    //    ESP_LOGI(TAG, "Servo X: %d, Y: %d", servoX_data, servoY_data);
       ESP_LOGI(TAG, "ServoSwitch: %d, grabButton: %d", servoSwitch, grabButton);
    }
}
