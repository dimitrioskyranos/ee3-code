...
static void uart_init(void) {
    ...
    uart_driver_install(UART_NUM, BUF_SIZE * 16, BUF_SIZE * 16, QUEUE_SIZE, &uart_queue, 0);
    ...
    void app_main(void)
    {
        ...
        ESP_LOGI(TAG,"Start to transmit the pictures");    
                ...
                if (event.type == UART_DATA) {
                    ...
                    if (len > 0) {
                        if (temp_len + len >= BUF_SIZE) {
                            ...
                            uart_flush_input(UART_NUM);  // <- clear RX buffer
                            temp_len = 0;
